...
public class GuiEditSign extends GuiScreen {
    ...
    private Deque<String[]> undoStack = new ArrayDeque<>(); // 备份被清空字符
    ...
    // 布局参数
    private static final int TITLE_Y_OFFSET = -100;  // 标题距离中心偏移（负值向上）
    private static final int BUTTON_Y_OFFSET = 40;  // 按钮距离屏幕底部偏移量
    private static final int BUTTON_WIDTH = 200;   // 按钮宽度
    private static final int BUTTON_HEIGHT = 20;   // 按钮高度

    private final int BACKGROUND_WIDTH;    // 输入框宽度
    private static final int SIGN_BACKGROUND_WIDTH = 200;            // 普通告示牌输入框宽度
    private static final int HANGING_SIGN_BACKGROUND_WIDTH = 140;    // 悬挂告示牌输入框宽度
    private static final int TEXT_COLOR = 0xFF000000;   // 文字颜色
    private static final int LINE_SPACING = 16;         // 行间距
    private static final int TEXT_FIELD_HEIGHT = 14;    // 输入框高度

    // 计算文本位于贴图的位置
    private static final float CONTENT_OFFSET_FACTOR = 0.2F; // 内容区域占贴图高度的比例
    private static final int INPUT_FIELD_Y_OFFSET = 35;     // 输入框相对贴图顶部的偏移量
    ...
    public GuiEditSign(HasBackSideSignTileEntity tile) {
        ...
        // 获取背景贴图
        Map.Entry<String, String> entry = tile.getEditedBlockType().entrySet().iterator().next();
        this.BACKGROUND_TEXTURE = new ResourceLocation(SuiKe.MODID, "textures/gui/" + entry.getKey() + "/" + entry.getValue() + ".png");

        this.BACKGROUND_WIDTH = entry.getKey().contains("hanging_sign")  ? HANGING_SIGN_BACKGROUND_WIDTH : SIGN_BACKGROUND_WIDTH;
    ...
    public void initGui() {
        super.initGui();
        ITextComponent[] text = this.tile.getTextForEditing(editingSlot);
        ...
        this.buttonList.add(new GuiButton(0, halfScreen - BUTTON_WIDTH/2, buttonY + 22, BUTTON_WIDTH, BUTTON_HEIGHT, I18n.format("gui.done")));
        this.buttonList.add(new GuiButton(1, halfScreen - BUTTON_WIDTH/2, buttonY, BUTTON_WIDTH/2 - 1, BUTTON_HEIGHT, "×"));
        this.buttonList.add(new GuiButton(2, halfScreen + 2, buttonY, BUTTON_WIDTH/2 - 1, BUTTON_HEIGHT, "↺"));
    ...
    protected void keyTyped(char typedChar, int keyCode) throws IOException {
        ...
        if (fontRenderer.getStringWidth(testText) > maxTextWidth) {
            return; // 阻止输入
        }
    ...
    protected void actionPerformed(GuiButton button) throws IOException {
        ...
        else if (button.id == 2 && !this.undoStack.isEmpty()) {
            // 撤回操作
