...
// 存储渲染位置
    ...
    public RenderPosition getItemRenderPos(int slot) {
        if (slot == 0)
    ...
    public void setItemRenderPos(int slot, RenderPosition renderPos) {
        if (slot == 0)
...
// 获取渲染位置
    public RenderPosition getItemRenderPosition(int slot) {
        ...
        if (block instanceof ModBlockHangingSign || ((ModBlockHangingSignAttached) block).isCardinalFacing(state, block.getMetaFromState(state))) {
            ...
            if (isNorthSouth) {
                if (slot == 0) {
                    this.setItemRenderPos(slot, ROTATION_MAP.get(0)); // 北方
                } else {
                    this.setItemRenderPos(slot, ROTATION_MAP.get(8)); // 南方
                }
            } else {
                if (slot == 0) {
                    this.setItemRenderPos(slot, ROTATION_MAP.get(12)); // 西方
                } else {
                    this.setItemRenderPos(slot, ROTATION_MAP.get(4)); // 东方 
                }
            }
        ...
        // 默认返回: 北方 0 槽
        return ROTATION_MAP.get(0);
    }

// 渲染位置
    private static final Map<Integer, RenderPosition> ROTATION_MAP = ImmutableMap.<Integer, RenderPosition>builder()
        .put(0, new RenderPosition(0.5, 0.5781, 180f))
        .put(1, new RenderPosition(0.4701, 0.5722, 157.5f))
        .put(2, new RenderPosition(0.4448, 0.5552, 135f))
        .put(3, new RenderPosition(0.4278, 0.5299, 112.5f))
        .put(4, new RenderPosition(0.4219, 0.5, 90f))
        .put(5, new RenderPosition(0.4278, 0.4701, 67.5f))
        .put(6, new RenderPosition(0.4448, 0.4448, 45f))
        .put(7, new RenderPosition(0.4701, 0.4278, 22.5f))
        .put(8, new RenderPosition(0.5, 0.4219, 0f))
        .put(9, new RenderPosition(0.5299, 0.4278, -22.5f))
        .put(10, new RenderPosition(0.5552, 0.4448, -45f))
        .put(11, new RenderPosition(0.5722, 0.4701, -67.5f))
        .put(12, new RenderPosition(0.5781, 0.5, 270f))
        .put(13, new RenderPosition(0.5722, 0.5299, -112.5f))
        .put(14, new RenderPosition(0.5552, 0.5552, -135f))
        .put(15, new RenderPosition(0.5299, 0.5722, -157.5f))
        .build();
