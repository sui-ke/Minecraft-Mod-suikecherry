...
public class DecoratedPotTileEntity extends TileEntity implements ITickable {
    ...
    public boolean canAdd(ItemStack heldItem) {
        return this.storedItem.getCount() < this.storedItem.getMaxStackSize()
            && ItemStack.areItemsEqual(this.storedItem, heldItem)
            && this.storedItem.getMetadata() == heldItem.getMetadata()
            && ItemStack.areItemStackTagsEqual(this.storedItem, heldItem);
    }
