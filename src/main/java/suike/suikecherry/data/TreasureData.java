...
public class TreasureData {
    ...
    public TreasureData(Item treasure, float chance) {
        this((treasure != null ? treasure.getRegistryName().toString() : ""), 0, chance);
    }
    public TreasureData(ItemStack treasure, float chance) {
        this(treasure.getItem().getRegistryName().toString(), treasure.getMetadata(), chance);
    }
    ...
    public boolean isValid() {
        ...
        if (!isValid) {
            SuiKe.LOGGER.info("×××: {} - {}", this.treasure, this.meta);
        }
    ...
    public enum Structure {
        ...
        private static void log(Structure structure) {
            for (int i = 0; i < structure.treasureList.size(); i++) {
                ...
                if (data.isValid()) {
                    SuiKe.LOGGER.info("○○○: {}. {} - {} - ({}%)", i+1, data.getTreasure(), data.getMeta(), String.format("%.2f", chance));
                }
                else {
                    SuiKe.LOGGER.info("×××: {}. {} - {} - ({}%)", i+1, data.getTreasure(), data.getMeta(), String.format("%.2f", chance));
                }
