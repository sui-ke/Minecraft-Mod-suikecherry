...
public class SItemPetals extends Item implements SItem {
    ...
    private Block block; //落英对应的方块
    public void setBlock(Block block) {
        this.block = block; //设置落英的方块
    }

//使用物品
    @Override
    public EnumActionResult onItemUse(EntityPlayer player, World world, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        /*点击方块的状态*/IBlockState state = world.getBlockState(pos);
        /*点击的方块*/Block block = state.getBlock();

        if (block instanceof SBlockPetals) {
        //升级落英
            if (/*升级落英*/upPetals(world, pos, block)) {
        ...
        //首次放置落英
            if (facing == EnumFacing.UP) {// 仅允许在上方放置
                if (block == Blocks.GRASS ||
                    ...
                    if (/*放置落英*/placePetals(player, world, pos.up(), this.block)) {
                        /*消耗一个物品*/player.getHeldItem(hand).shrink(1);
                        return EnumActionResult.SUCCESS;
                    }
                }
...
//升级落英
    ...
    public static void playSound(World world, BlockPos pos) {
        ...
        /*播放音效*/world.playSound(null, pos, new SoundEvent(new ResourceLocation(SuiKe.MODID, "block.cherryleaves.place" + a)), SoundCategory.BLOCKS, 1.0F, 1.0F);
    }
}