...
import suike.suikecherry.packet.packets.SnifferUpStatePacket;
...
public class EntitySniffer extends EntityAnimal {
    ...
    private static final DataParameter<Byte> STATE = EntityDataManager.createKey(EntitySniffer.class, DataSerializers.BYTE);
    private static final DataParameter<Boolean> RIDING = EntityDataManager.createKey(EntitySniffer.class, DataSerializers.BOOLEAN);
    private static final DataParameter<Integer> DIG_TIME = EntityDataManager.createKey(EntitySniffer.class, DataSerializers.VARINT);
    ...
    protected void initEntityAI() {
        ...
        /*游泳*/this.tasks.addTask(4, new EntityAISwimming(this));
    ...
    public void onUpdate() {
        ...
        if (this.world.isRemote) {
            ...
            if (sniffingCooldown > 0) sniffingCooldown--;
        ...
        if (this.getState() == State.IDLING && rand.nextFloat() < 0.004F) {
...
// 交互
    ...
    public boolean processInteract(EntityPlayer player, EnumHand hand) {
        if (!this.world.isRemote && !this.isBreedingItem(player.getHeldItem(hand))) {
...
// 繁殖
    ...
    public boolean canMateWith(EntityAnimal other) {
        boolean a = other != this && other instanceof EntitySniffer
    ...
    public EntityAgeable createChild(EntityAgeable ageable) {
        this.world.spawnEntity(new EntityItem(
            ...
            this.posY,
            this.posZ,
            new ItemStack(BlockBase.SNIFFER_EGG) // 生成嗅探兽蛋
        ));
...
// 幼年
    ...
    public void setGrowingAge(int age) {
        ...
        if (age == 0) setSnifferSize();
    }

    public static EntitySniffer getChild(World world) {
        EntitySniffer sniffer = new EntitySniffer(world);
...
// 获取
    public double getMoveSpeed() {
        if (this.isChild()) return snifferMoveSpeed * 0.8;
...
// 状态
    ...
    public void upState(State state) {
        // 客户端修改状态
    ...
    public void setState(State state) {
        ...
        PacketHandler.INSTANCE.sendToAllAround(
            new SnifferUpStatePacket(this.getEntityId(), state),
    ...
    private void reloadCooldown() {
        switch(this.getState()) {
            case SNIFFING:
                sniffingCooldown = SNIFFING_COOLDOWN; // 嗅探状态进入冷却
            case DIGGING:
