        ...
        public SmithingTableContainer(EntityPlayer player) {
            ...
            for (int row = 0; row < 3; ++row) {
                for (int col = 0; col < 9; ++col) {
                    this.addSlotToContainer(new Slot(player.field_71071_by, col + row * 9 + 9, 
            ...
            for (int slot = 0; slot < 9; ++slot) {
                this.addSlotToContainer(new Slot(player.field_71071_by, slot, 
    ...
    // 减少所有输入槽物品
        ...
        public ItemStack slotClick(int slotId, int dragType, ClickType clickType, EntityPlayer player) {
            if (slotId == 3) {
                ...
                if (clickType == ClickType.SWAP || !player.field_71071_by.getItemStack().isEmpty()) {
                    return ItemStack.EMPTY;
                }
            }
...
// 下界合金配方
    ...
    private static void addNetheriteRecipe() {
        if (Examine.FuturemcID || Examine.UNBID) {
            addRecipe("diamond_sword", "netherite_sword");
            addRecipe("diamond_pickaxe", "netherite_pickaxe");
            addRecipe("diamond_axe", "netherite_axe");
            addRecipe("diamond_shovel", "netherite_shovel");
            addRecipe("diamond_hoe", "netherite_hoe");
            addRecipe("diamond_helmet", "netherite_helmet");
            addRecipe("diamond_chestplate", "netherite_chestplate");
            addRecipe("diamond_leggings", "netherite_leggings");
            addRecipe("diamond_boots", "netherite_boots");
        }
    }
    public static void addRecipe(String a, String b) {
