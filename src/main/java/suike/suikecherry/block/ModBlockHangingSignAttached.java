...
// 方块更新
    ...
    public void spawnDrop(World world, BlockPos pos) {
        HangingSignTileEntity tile = (HangingSignTileEntity) world.getTileEntity(pos);
        this.spawnAsEntity(world, pos, tile.getItemHandler().getStackInSlot(0));
        this.spawnAsEntity(world, pos, tile.getItemHandler().getStackInSlot(1));
    }
...
// 方块交互
    ...
    public boolean onBlockActivated(World world, BlockPos clickPos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing clickFacing, float hitX, float hitY, float hitZ) {
        ...
        // 点击下方放置灯笼逻辑
        if (clickFacing == EnumFacing.DOWN) return Lantern.lantern(world, clickPos.down(), player, hand, 1);
        ...
        if (heldItem.isEmpty()) {
            // 编辑文字
            ((HasBackSideSignTileEntity) tile).setEditedSlot(slot);
            return ((HasBackSideSignTileEntity) tile).editText(player);
        }
        // 这一面什么都没有
...
// 获取方向 编辑的面
    ...
    private int getSlotFromFacings(int axis, EnumFacing clickFacing, EntityPlayer player) {
