...
// 收获
    ...
    public void harvestBlock(World world, EntityPlayer player, BlockPos pos, IBlockState state, TileEntity tile, ItemStack stack) {
        if (!world.isRemote && tile instanceof DecoratedPotTileEntity) {
...
// 弹射物破坏
    ...
    // 检查弹射物
    public static List<Class<? extends Entity>> EXTRA_PROJECTILE_CLASSES = Lists.newArrayList(
...
// 掉落物品
    ...
    private void breakPot(World world, BlockPos pos, String[] sherdIDs) {
        for (String potteryType : sherdIDs) {
            Item item = "brick".equals(potteryType) ? 
                Items.BRICK : 
    ...
    private void dropStoredItem(World world, BlockPos pos, DecoratedPotTileEntity potTile) {
        if (potTile.hasStoredItem()) {
            spawnAsEntity(world, pos, potTile.getStoredItem());
        }
    }
