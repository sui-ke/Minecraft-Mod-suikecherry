...
//落英类
public class ModBlockPetals extends BlockCarpet {
    public ModBlockPetals(String name) {
        ...
        /*添加到ITEMS列表*/BlockBase.ITEMBLOCKS.add(new ModItemPetals(name, this).setRegistryName(this.getRegistryName()));
...
//检查下方方块
    public static boolean downBlockIsGrass(World world, BlockPos pos) {
        return downBlockIsGrass(world, pos, "isPlayer");
    }
    public static boolean downBlockIsGrass(World world, BlockPos pos, String way) {
        IBlockState downBlockState = world.getBlockState(pos.down());
        Block downBlock = downBlockState.getBlock();

        if (!ConfigValue.petalsUnlimitedPlace || "isGenerate".equals(way)) {
            return downBlock == Blocks.DIRT ||
                    downBlock == Blocks.GRASS ||
                    downBlock == Blocks.MYCELIUM ||
                    downBlock == Blocks.FARMLAND ||
                    (
                        downBlock.getRegistryName().toString().contains("moss") &&
                        downBlock.isFullCube(downBlockState)
                    );
        }
        else {
            AxisAlignedBB collisionBox = downBlockState.getCollisionBoundingBox(world, pos.down());
            if (collisionBox != null && (collisionBox.maxX - collisionBox.minX) >= 1.0
                                    && (collisionBox.maxZ - collisionBox.minZ) >= 1.0
                                    && (collisionBox.maxY - collisionBox.minY) >= 1.0) {
                return true;
            }
        }
