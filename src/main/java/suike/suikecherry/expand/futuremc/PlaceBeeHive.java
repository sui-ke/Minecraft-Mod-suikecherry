...
public class PlaceBeeHive {
    ...
    private static final BeeHiveBlock beeNestBlock = (BeeHiveBlock) Block.getBlockFromItem(Item.getByNameOrId("futuremc:bee_nest"));
    private static final IBlockState beeNestBlockstate = beeNestBlock.getDefaultState();
        ...
        for (int x = -2; x <= 2; x++) {
            for (int y = 0; y <= 1; y++) {
                for (int z = -2; z <= 2; z++) {
                    ...
                    if (world.isAirBlock(checkPos) && isAdjacentToCherryLog(world, checkPos)) { // 如果是空气
