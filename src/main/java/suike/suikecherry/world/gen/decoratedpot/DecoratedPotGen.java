...
public class DecoratedPotGen {
    ...
    private static String[] getDefaultPotteryTypes() {
        if (DEFAULT_POTTERY_TYPES == null) {
            DEFAULT_POTTERY_TYPES = ModItemPotterySherd.getAllPotteryType().toArray(new String[0]);
        }
    ...
    public static void placedPot(World world, BlockPos pos) {
        // 获取所有纹饰类型
        placedPot(world, pos, getDefaultPotteryTypes());
    }
    ...
    private static String[] getPotSherdIDs(Random rand, String... potteryTypes) {
        if (potteryTypes == null || potteryTypes.length == 0) {
            // 使用默认纹饰列表
            potteryTypes = getDefaultPotteryTypes();
        }
