...
public class CherryBiome extends Biome {
    ...
    public CherryBiome() {
        ...
        if (ConfigValue.cherryBiomeSpawnBamboo && ConfigValue.hasBambo) {
...
/*  this.decorator
    ...
    extraTreeChance       额外树木生成几率
    flowersPerChunk       每个区块花朵数量
    grassPerChunk         每个区块草丛数量
    deadBushPerChunk      每个区块枯树数量
    mushroomsPerChunk     每个区块蘑菇数量
    reedsPerChunk         每个区块甘蔗数量
    cactiPerChunk         每个区块仙人掌数量
    gravelPatchesPerChunk 每个区块沙砾块数量
    sandPatchesPerChunk   每个区块沙子块数量
    clayPerChunk          每个区块粘土数量
    bigMushroomsPerChunk  每个区块大蘑菇数量
    generateFalls         生成瀑布
    decorating            装饰方法
    chunkPos              区块位置
    chunkProviderSettings 区块生成设置
    clayGen               粘土生成器
    sandGen               沙子生成器
    gravelGen             沙砾生成器
    dirtGen               土壤生成器
    ...
    lapisGen              青金石矿生成器
    flowerGen             花朵生成器
    mushroomBrownGen      棕色蘑菇生成器
    mushroomRedGen        红色蘑菇生成器
    bigMushroomGen        大蘑菇生成器
    reedGen               甘蔗生成器
    cactusGen             仙人掌生成器
    waterlilyGen          睡莲生成器
    waterlilyPerChunk     每个区块睡莲数量
    treesPerChunk         每个区块树木数量
    ...
    private class CherryTreeGenerator implements ICherryBiomeDecorate {
        ...
        public void decorate(World world, BlockPos pos, Random random) {
            ...
            for (int i = 0; i < this.TREEGENPOS.size(); i++) {
                ...
                // 为每个位置创建一个随机数生成器
                posRandom.setSeed(world.getSeed() + treePos.getX() * 4 + treePos.getZ() * 7 + 1);
...
// 竹子生成器
    private class BambooGenerator implements ICherryBiomeDecorate {
        ...
        public BambooGenerator() {
            ...
            int defaultMate = 3;
            int smallLeavesMate = 4;
            int largeLeavesMate = 5;
            Block bambooBlock = Block.getBlockFromName("futuremc:bamboo");
            Object BOPBambooGenerator = null;

            if ("sakura".equals(ConfigValue.bambo)) {
                defaultMate = 0;
                smallLeavesMate = 0;
                largeLeavesMate = 0;
                bambooBlock = Block.getBlockFromName("sakura:bamboo");
            }
            else if ("biomes".equals(ConfigValue.bambo)) {
                bambooBlock = null;
                BOPBambooGenerator = new GeneratorBulbTree.Builder()
                    .minHeight(10)
                    .maxHeight(20)
            ...
            boolean has = bambooBlock != null;
            this.defaultBamboo = has ? bambooBlock.getStateFromMeta(defaultMate) : null;
            this.smallLeaves = has ? bambooBlock.getStateFromMeta(smallLeavesMate) : null;
            this.largeLeaves = has ? bambooBlock.getStateFromMeta(largeLeavesMate) : null;
            this.BOPBambooGenerator = BOPBambooGenerator;
        }
        ...
        private void generateDirtSimple(World world, Random random, BlockPos pos) {
            ...
            // 额外放置灰化土
            for (int i = 1; i <= am; i++) {
                random.setSeed(world.getSeed() + pos.getX() * 4 + pos.getZ() * 7 + i);
                // 随机获取一个数组
