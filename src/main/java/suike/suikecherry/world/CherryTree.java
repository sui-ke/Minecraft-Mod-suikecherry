        ...
        for (int x = -r; x <= r; x++) {// r 生成半径
            for (int z = -r; z <= r; z++) {
                if (x * x + z * z <= r * r) {// 判断是否在圆内
                    ...
                    if (currentY > (y + 10) || currentY < (y - 10)) {
                        continue;
                    }
                    ...
                    if (!isSapling) {
                        if (旧方块 == Blocks.TALLGRASS) {
                            //清除草
                            placeBlock(currentPos, air, true);
                        } else if (旧方块 == Blocks.DOUBLE_PLANT) {
                            //清除高草
                            placeBlock(currentPos, air, true);
                            placeBlock(currentPos.up(), air, true);
                        }
                    }
                    ...
                    if (旧方块 == Blocks.AIR || 旧方块 == Blocks.TALLGRASS || 旧方块 == Blocks.DOUBLE_PLANT) {
...
//放置方块
    ...
    public static void placeBlock(BlockPos pos, IBlockState state, boolean a) {
        ...
        if (block == Blocks.AIR || block == BlockBase.CHERRY_LEAVES || a) {
            world.setBlockState(pos, state, 2);
            blockUpdate(pos, state);
        }
    }

    //通知相邻方块更新
    private static void blockUpdate(BlockPos pos, IBlockState state) {
        if (state.getBlock() == BlockBase.CHERRY_LOG) {
...
//生成蜂巢和蜜蜂
    ...
    //生成蜂巢
        ...
        //设置方块状态并放置
            ...
            /*放置方块*/world.setBlockState(pos, state, 2);

        //获取蜜蜂的实体, 并生成
            BlockPos spawnPos = pos.offset(facing).add(1.5, -0.5, 1.5);

            //防止蜜蜂窒息
            if (world.getBlockState(spawnPos).getBlock() == Blocks.AIR) {
                //生成3只蜜蜂
                for (int i = 1; i <= 3; i++) {
                    Entity bee = new EntityBee(world);
                    ...
                    //为蜜蜂绑定蜂巢
                        ...
                        hivePosField.set((EntityBee) bee, pos);
...
//播放音效方法
    private static void playSound(World world, BlockPos pos) {
        /*生成随机数*/int a = (int) (Math.random() * 5) + 1;
        /*播放音效*/world.playSound(null, pos, new SoundEvent(new ResourceLocation(SuiKe.MODID, "block.cherryleaves.place" + a)), SoundCategory.BLOCKS, 3.0F, 1.0F);
    }
}