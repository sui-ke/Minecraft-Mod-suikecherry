...
public class ConfigRead {
    public static void config() {
        ...
        ConfigValue.cherryBiomeSize = readConfig("Cherry", "cherryBiomeSize", 1, 16);
    ...
    private static void readBamboConfig() {
        ...
        Map<Integer, String> modPriorities = new TreeMap<>();
        if (hasFuturemc) {
            int priority = readConfig("Cherry.Bamboo", "futuremc", 0, 2);
            modPriorities.put(priority, "futuremc");
        }
        if (hasBiomesoplenty) {
            int priority = readConfig("Cherry.Bamboo", "biomesoplenty", 1, 2);
            modPriorities.put(priority, "biomesoplenty");
        }
        if (hasSakura) {
            int priority = readConfig("Cherry.Bamboo", "sakura", 2, 2);
            modPriorities.put(priority, "sakura");
        }

        if (!modPriorities.isEmpty()) {
            ConfigValue.hasBambo = true;
            ConfigValue.bambo = modPriorities.entrySet().iterator().next().getValue();
        }
    }
